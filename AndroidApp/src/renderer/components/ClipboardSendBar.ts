/**
 * ClipboardSendBar.ts — Android version.
 * Feature 2: Clipboard text sharing.
 * Compact bar below the file picker. Visible when ≥1 device is selected.
 * Uses AppBridge.sendClipboard() instead of IpcClient.
 */
...
export class ClipboardSendBar extends Component {
  ...
  protected onMount(): void {
    ...
    // Show/hide based on device selection — patch in-place, no re-render
    const unsub = StateManager.subscribe('selectedDeviceIds', () => this.updateVisibility());
  ...
  private async handleSend(): Promise<void> {
    ...
    try {
      await Promise.all(
        selectedIds.map(deviceId => AppBridge.sendClipboard({ deviceId, text: safeText })),
      );
