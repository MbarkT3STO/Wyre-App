/**
 * Toast.ts — identical to desktop version.
 */
...
export class Toast extends Component {
  ...
  protected onMount(): void {
    ...
    this.element.querySelector('.toast__close')?.addEventListener('click', () => this.dismiss());
    ...
    if (actionBtn && this.options.onAction) {
      ...
      actionBtn.addEventListener('click', () => { action(); this.dismiss(); });
    }
  }
  ...
  dismiss(): void {
    ...
    if (this.element) {
      ...
      setTimeout(() => { this.unmount(); this.onDismiss?.(); }, 300);
    }
  }
