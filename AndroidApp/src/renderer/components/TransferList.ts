/**
 * TransferList.ts — adapted for Android (uses AppBridge for clearHistory).
 */
...
import type { TransferQueueUpdatedEvent } from '../../bridge/WyrePlugin';
...
type QueueItem = TransferQueueUpdatedEvent['queue'][number];
...
export class TransferList extends Component {
  ...
  protected onMount(): void {
    const unsub1 = StateManager.subscribe('activeTransfers', (transfers) => this.syncActive(transfers));
    this.addCleanup(unsub1);

    const unsubQueue = StateManager.subscribe('sendQueue', (queue) => this.syncQueue(queue));
    this.addCleanup(unsubQueue);

    if (!this.activeOnly) {
      const unsub2 = StateManager.subscribe('transferHistory', (history) => this.syncHistory(history));
      this.addCleanup(unsub2);

      const clearBtn = this.element?.querySelector('.transfer-list__clear-btn');
      if (clearBtn) {
        clearBtn.addEventListener('click', () => {
          void AppBridge.clearHistory().then(() => StateManager.setState('transferHistory', []));
        });
      }
  ...
  private syncActive(transfers: Map<string, Transfer>): void {
    ...
    for (const [id, item] of this.activeItems) {
      ...
      if (!current || !activeStatuses.has(current.status)) { item.unmount(); this.activeItems.delete(id); }
    }
  ...
  private syncQueue(queue: QueueItem[]): void {
    ...
    if (queue.length === 0) { this.queueSection.style.display = 'none'; container.innerHTML = ''; return; }
    ...
    container.innerHTML = queue.map((item, idx) => `
      <div class="transfer-queue-item">
        ...
        <div class="transfer-queue-item__icon"><i class="fa-solid fa-arrow-up"></i></div>
        <div class="transfer-queue-item__meta">
          <span class="transfer-queue-item__name">${escapeHtml(item.fileName)}</span>
  ...
  private syncHistory(history: TransferRecord[]): void {
    ...
    for (const [id, item] of this.historyItems) {
      if (!newIds.has(id)) { item.unmount(); this.historyItems.delete(id); }
    }
